-- Phase 2 Security Step 1: Audit Log Immutability (修正版)
-- 既存機能を破壊せず、監査ログの不変性を実現

-- 1. 既存のアーカイブテーブルとポリシーをクリーンアップ
DROP POLICY IF EXISTS "Admins can view archived logs" ON public.activity_logs_archive;
DROP TABLE IF EXISTS public.activity_logs_archive;

-- 2. アーカイブテーブル作成（古いログの保管用）
CREATE TABLE public.activity_logs_archive (
  id uuid PRIMARY KEY,
...
-- 3. activity_logsのDELETE/UPDATEポリシーを削除（不変性確保）
